...
const skeletonJson = new URL("/skeleton.json", import.meta.url).href;
const skeletonAtlas = new URL("/skeleton.atlas", import.meta.url).href;
...
// const skeletonJson = '/skeleton.json';
// const skeletonAtlas = '/skeleton.atlas';
...
const makeAnimation = () => {
  ...
  game.animations.spine.current =
    game.animations.spine.current >= 2 || game.animations.spine.current === -1
      ? 0
      : game.animations.spine.current + 1;
  const animationInfo =
    game.animations.spine.timing[game.animations.spine.current];
...
const start = async (track_id) => {
  ...
  if (selectedIndex === -1)
    return showPopup("error", "The track you have selected was not found");
  const track = game.tracks[selectedIndex];
  if (track.next_timer > 0)
    return showPopup(
      "error",
      "You can listen to this track via " + secondsToHMS(track.next_timer)
    );
  game.starting.status = true;
  game.starting.timeout = setTimeout(() => {
    store.dispatch("setDropGameStarted", true);
    game.track.info = track;
    game.track.interval = setInterval(
      () => updateTrackInfo(track.manager),
      100
    );
    track.manager.play();
...
const tap = async (e) => {
  ...
  if (game.paused || game.audio.fade.interval) {
    ...
    game.track.info.manager.seek(
      amount > currentSeconds ? 0 : currentSeconds - amount
    );
...
const makeAltAnimation = () => {
  ...
  const align = getRandomInt(1, 4),
    sizes = ["s", "m", "l", "xl"];
  const wordAlign =
    game.animations.repeat.left >= game.animations.repeat.max
      ? "right"
      : game.animations.repeat.right >= game.animations.repeat.max
      ? "left"
      : align === 1 || align === 3
      ? "left"
      : "right";
  if (game.animations.repeat.left >= game.animations.repeat.max) {
    game.animations.repeat.left = 0;
  } else if (game.animations.repeat.right >= game.animations.repeat.max)
    game.animations.repeat.right = 0;
  const leftAlign = getRandomInt(
      wordAlign === "right" ? 75 : 5,
      wordAlign === "right" ? 95 : 20
    ),
    topAlign = getRandomInt(30, 45),
    rotateDeg = getRandomInt(
      wordAlign === "right" ? 10 : 335,
      wordAlign === "right" ? 25 : 350
    );
  ...
  return setTimeout(() => {
    game.animations.items = game.animations.items.filter(
      (animation) => animation.uuid !== animationUuid
    );
  }, 800);
};
